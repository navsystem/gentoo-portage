...
inherit autotools bash-completion-r1 flag-o-matic linux-info tmpfiles udev systemd git-r3

DESCRIPTION="mirror/replicate block-devices across a network-connection"
HOMEPAGE="https://www.linbit.com/drbd"
EGIT_REPO_URI="https://github.com/LINBIT/drbd-utils.git"
...
src_prepare() {
	...
	sed -e "s:@prefix@/lib:@prefix@/$(get_libdir):" \
		...
		-i user/*/Makefile.in || die
	sed -i -e "s/lib/$(get_libdir)/" scripts/drbd.service.in || die
...
src_compile() {
	...
	emake CXXFLAGS="${CXXFLAGS}" OPTFLAGS="${CFLAGS}" tools
}
