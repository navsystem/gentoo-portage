...
inherit autotools-utils readme.gentoo systemd toolchain-funcs git-r3
...
IUSE="debug gdm +gtk +pango static-libs"
...
src_prepare() {
	...
	sed -i "s:SYSTEMD_UNIT_DIR=.*\$:SYSTEMD_UNIT_DIR=$(systemd_get_unitdir):g" \
		configure.ac || die 'sed configure.ac failed'
	sed -i "/DESTDIR.*plymouthdrundir/d" src/Makefile.am || die
