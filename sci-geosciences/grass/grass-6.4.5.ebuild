...
inherit eutils gnome2 multilib python versionator wxwidgets autotools fdo-mime
...
PATCHES=(
	...
	"${FILESDIR}"/${P}-ffmpeg-includes.patch
)
...
src_prepare() {
	...
	epatch ${PATCHES[@]}
	epatch_user
	use wxwidgets || sed -e "/^SUBDIRS/s/ wxpython//" -i ${S}/gui/Makefile
	eautoreconf
}
